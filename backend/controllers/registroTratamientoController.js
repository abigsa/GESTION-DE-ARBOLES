...
const validarFechaNoFutura = (fecha, nombreCampo) => {
  if (!fecha) return `${nombreCampo} es obligatoria.`;
  ...
  if (isNaN(fechaIngresada.getTime())) return `${nombreCampo} no es válida.`;
  if (fechaIngresada > hoy) return `${nombreCampo} no puede ser futura.`;
...
// QUERY BASE
// ----------------------------------------------------------
const baseRegistroTratamientoQuery = `
  SELECT
    RT.ID_REGISTRO,
    RT.ID_ARBOL,
...
const insertar = async (req, res) => {
  ...
  try {
    ...
    await conn.execute(
      `
      BEGIN
        PKG_REGISTRO_TRATAMIENTO.INSERTAR(
          :id_arbol,
          :id_tipo_tratamiento,
          :id_fertilizante,
          :fecha_aplicacion,
    ...
    res.status(201).json({
      success: true,
      message: 'Registro de tratamiento insertado correctamente.',
    });
  } catch (err) {
...
const actualizar = async (req, res) => {
  ...
  try {
    ...
    await conn.execute(
      `
      BEGIN
        PKG_REGISTRO_TRATAMIENTO.ACTUALIZAR(
          :id_registro,
          :id_arbol,
          :id_tipo_tratamiento,
          :id_fertilizante,
    ...
    res.status(200).json({
      success: true,
      message: 'Registro de tratamiento actualizado correctamente.',
    });
  } catch (err) {
    res.status(500).json({ success: false, message: err.message });
  } finally {
...
const eliminar = async (req, res) => {
  ...
  try {
    ...
    await registrarAuditoria(conn, {
      ...
      operacion: 'DELETE',
      idRegistro: null,
      descripcion: 'Registro eliminado en REGISTRO_TRATAMIENTO',
      usuarioId: null,
    ...
    res.status(200).json({
      success: true,
      message: 'Registro de tratamiento eliminado correctamente.',
    });
  } catch (err) {
    res.status(500).json({ success: false, message: err.message });
  } finally {
...
const obtenerPorId = async (req, res) => {
  ...
  try {
    ...
    const result = await conn.execute(
      `
      ${baseRegistroTratamientoQuery}
        AND RT.ID_REGISTRO = :id_registro
      ORDER BY RT.ID_REGISTRO
      `,
      { id_registro: Number(id_registro) },
      { outFormat: oracledb.OUT_FORMAT_OBJECT }
    ...
    if (rows.length === 0) {
      return res.status(404).json({
        success: false,
        message: 'Registro de tratamiento no encontrado.',
      });
    }
